-- Demande utilisateur : "au moment de la distribution des rôles, les autres
-- loups doivent aussi savoir qui est le loup alpha si il y en a."
--
-- Bug trouvé : wolf_teammates ne listait QUE les 'loup_garou' (jamais le
-- 'loup_alpha'), et surtout n'était calculé QUE si v_my_role = 'loup_garou'
-- — ce qui veut dire que le Loup Alpha lui-même ne voyait JAMAIS ses
-- coéquipiers (WolfPackList.tsx se masque entièrement si la liste est
-- vide). Un Loup-Garou simple, lui, voyait les autres Loups-Garous simples
-- mais jamais l'Alpha.
--
-- Correctif : la clause couvre désormais 'loup_garou' ET 'loup_alpha' des
-- deux côtés (qui peut voir / qui est listé). Nouveau champ wolf_alpha_id
-- (id du Loup Alpha parmi les coéquipiers listés, ou null s'il n'y en a
-- pas/plus) pour que le client puisse le distinguer visuellement dans la
-- liste (badge), sans changer la forme de wolf_teammates (encore un simple
-- tableau d'ids, pour ne pas casser son usage existant dans WolfPanel).
...
begin
  ...
  select jsonb_build_object(
    ...
    -- CORRIGÉ : couvre désormais 'loup_garou' ET 'loup_alpha', des deux
    -- côtés — avant, seul un Loup-Garou simple voyait des coéquipiers
    -- (jamais l'Alpha inclus dedans), et l'Alpha lui-même n'en voyait
    -- jamais aucun.
    ...
    -- Nouveau : permet au client de distinguer l'Alpha dans la liste des
    -- coéquipiers ci-dessus (badge). null si pas de Loup Alpha dans cette
    -- partie, ou si le viewer n'est pas lui-même un loup.
