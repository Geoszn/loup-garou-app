-- ============================================================================
-- Corrige une régression introduite par 0041_debate_extend_reply_ghost_
-- listen_night_recap.sql : sa version de get_my_game_view avait été
-- reconstruite à partir d'un ancien modèle (0027_vote_recap.sql) au lieu de
-- la dernière version réelle (0038_join_in_progress_games.sql), ce qui a
-- silencieusement :
--
--   1. FAIT DISPARAÎTRE le champ `join_requests` — l'hôte d'une partie
--      publique (ou d'une partie privée rejointe en cours par code) ne
--      voyait donc plus JAMAIS les demandes en attente, qui restaient
--      bloquées côté demandeur sans que personne n'y réponde jamais.
--      C'est le bug rapporté : "la demande reste en attente... le
--      modérateur ne reçoit aucune demande".
--
--   2. fait disparaître `captain_voter_id` dans `vote_recap` (le récap de
--      vote perdait l'indication du vote à double poids du Capitaine).
--
--   3. RÉINTRODUIT PAR ERREUR `little_girl_wolves` — un champ retiré
--      intentionnellement par 0032_remove_petite_fille_spy.sql (mécanique
--      d'espionnage de la Petite Fille jugée problématique pour l'équilibre
--      du jeu). Le client ne s'en sert plus depuis 0032, mais mieux vaut ne
--      plus l'exposer du tout côté serveur non plus.
--
-- Cette migration reprend la bonne base (0038) et y réapplique uniquement
-- les deux ajouts légitimes faits depuis (day_reveal_ready_ids côté 0041,
-- night_recap côté 0043_night_recap_headlines.sql).
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Ajouté par 0041 (récap de nuit "prêt à continuer") — conservé.
    ...
    -- Ajouté par 0043 (titres de la nuit, voir migration 0043) — conservé.
    ...
    -- RÉTABLI (avait disparu par erreur, voir en-tête de fichier) : sans ce
    -- champ, l'hôte ne voit plus jamais les demandes pour rejoindre une
    -- partie publique (ou une partie privée rejointe en cours par code).
    'join_requests', case
