-- ============================================================================
-- Demande utilisateur : "Quand la Voyante regarde la carte de quelqu'un, je
-- ne veux plus qu'elle ait accès au rôle précis de la personne. Elle saura
-- juste si la personne est Villageois ou Loup. Point final -- pas si elle
-- est Voyante, Petite Fille ou quoi que ce soit d'autre."
--
-- Seul point touché : le champ 'seer_reveals' de get_my_game_view (aucun
-- changement dans submit_voyante, qui n'enregistre déjà que la cible, jamais
-- son rôle -- la lecture du rôle se fait entièrement à la volée ici). Le
-- rôle exact de la cible est remplacé par son camp : 'loup_garou' si la
-- cible est Loup-Garou OU Loup Alpha (même regroupement que
-- check_and_apply_win, migration 0088), sinon 'villageois' -- ces deux
-- valeurs existent déjà comme rôles réels, donc roleLabel() côté client les
-- affiche correctement sans aucun changement de traduction nécessaire côté
-- seer_reveals.
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Retour utilisateur (migration 0098) : la Voyante ne découvre plus le
    -- rôle exact de sa cible, seulement son CAMP -- 'loup_garou' si la cible
    -- est Loup-Garou ou Loup Alpha (même regroupement que le calcul de
    -- victoire, check_and_apply_win), sinon 'villageois'. roleLabel() côté
    -- client affiche déjà ces deux rôles correctement, aucun changement de
    -- traduction requis pour ce champ.
