-- ----------------------------------------------------------------------------
-- Demande utilisateur : le mentor de l'Enfant Sauvage doit savoir QU'IL a
-- été désigné, mais jamais PAR QUI. get_my_game_view envoyait jusqu'ici
-- 'mentee_ids' — la liste des user_id des Enfants Sauvages ayant choisi ce
-- joueur comme mentor — directement au client, qui se contentait de
-- masquer le nom dans l'UI (NightRecapModal). L'identité restait donc
-- techniquement récupérable (outils de développement du navigateur) même
-- une fois l'affichage corrigé côté client. Remplacé par un simple
-- booléen 'chosen_as_mentor' : aucune identité n'est plus jamais envoyée.
-- ----------------------------------------------------------------------------
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Simple fait, jamais une identité (voir commentaire d'en-tête de cette
    -- migration) : le mentor sait qu'il a été désigné, jamais par qui.
