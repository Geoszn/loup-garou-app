-- ============================================================================
-- Demande utilisateur : "Quand l'Enfant Sauvage se transforme en Loup-Garou
-- après avoir perdu son mentor, le compte total du nombre de loups doit
-- changer pour ne pas tromper les gens dans la partie."
--
-- Même bug/patron que la migration 0095 (corrigé alors pour l'infection du
-- Loup Alpha) : RosterSummary.tsx calcule totalWolves à partir de
-- role_counts (composition INITIALE, connue de tous dès le salon d'attente)
-- + 1 si une infection Alpha a eu lieu -- mais rien n'était prévu pour la
-- conversion de l'Enfant Sauvage (migration 0069), qui elle aussi change le
-- nombre réel de loups en cours de partie sans jamais mettre à jour
-- role_counts.
--
-- Ajout d'un nouveau champ public 'wild_child_conversion_occurred', même
-- patron que 'alpha_infection_occurred' (migration 0095) : ne révèle PAS qui
-- s'est transformé (déjà annoncé publiquement de façon anonyme, voir
-- migration 0077), juste QUE ça a eu lieu quelque part dans la partie -- au
-- plus une fois par partie (un seul Enfant Sauvage possible dans le deck,
-- voir start_game).
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Public (voir en-tête de fichier) : au moins un Enfant Sauvage a-t-il
    -- rejoint les Loups-Garous à un moment de la partie ? Ne révèle ni qui,
    -- ni quand -- sert uniquement à corriger totalWolves côté client
    -- (RosterSummary.tsx), même principe que alpha_infection_occurred.
