-- CORRECTIF URGENT : get_my_game_view référençait `p_user_id` (inexistant —
-- seul p_game_id est un paramètre de cette fonction) dans le champ
-- 'seer_reveals', ligne `na.actor_id = p_user_id` au lieu de `v_user`.
-- Erreur introduite par erreur lors de la réécriture complète de la
-- fonction en migration 0093 (refonte du Loup Alpha).
--
-- Impact : PostgREST/Postgres résout tous les identifiants d'une requête au
-- moment de la PLANIFICATION, y compris ceux situés dans une branche CASE
-- WHEN non empruntée à l'exécution — donc cette erreur faisait échouer
-- get_my_game_view sur CHAQUE appel, pour TOUS les joueurs, quel que soit
-- leur rôle ou le statut de la partie (pas seulement pour la Voyante),
-- depuis le déploiement de la migration 0093 aujourd'hui. Toute partie en
-- cours ou nouvellement créée restait bloquée sur l'écran de chargement.
