-- ============================================================================
-- Élargit la fenêtre pendant laquelle une victime d'Anancy voit l'avis
-- privé "🕸️ Le destin a changé" (anancy_swapped_me) : jusqu'ici restreint
-- au statut 'day_reveal' uniquement, soit une fenêtre de 15-30s (voir
-- role_reveal_seconds) au moment précis où le bilan de nuit s'affiche —
-- manquée, l'info disparaissait pour de bon (get_my_game_view ne renvoyait
-- plus jamais true ensuite, même en rechargeant la page).
--
-- Demande utilisateur : "à chaque fois qu'Anancy échange les rôles des
-- joueurs, une notification devrait informer ces joueurs-là... sans
-- révéler avec qui". Le mécanisme existait déjà (NightRecapModal.tsx) mais
-- sa fenêtre de visibilité était trop courte et surtout non rejouable.
--
-- Corrigé : ne dépend plus du statut de la partie, seulement de
-- swapped_at_night = night_number — vrai tout le jour où l'échange prend
-- effet (bilan de nuit, débat, vote, dépouillement) ET la nuit qui suit
-- immédiatement, jusqu'à ce que la nuit suivante commence. Toujours aucune
-- indication de l'identité de l'autre joueur ni du nouveau rôle.
-- ============================================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Élargi (voir commentaire d'en-tête) : ne dépend plus du statut,
    -- reste vrai tout le jour de l'échange + la nuit qui suit, tant que la
    -- nuit suivante n'a pas encore commencé.
