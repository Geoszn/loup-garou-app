-- Demande utilisateur : le total de loups affiché dans RosterSummary.tsx
-- (bouton "📊 effectifs") reste figé sur la composition INITIALE
-- (role_counts), même une fois une infection du Loup Alpha réussie en cours
-- de partie — le villageois converti n'est jamais compté dans le total.
-- Ajoute un signal public permettant au client de corriger ce total.
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Fait public déjà annoncé à tout le monde via game_log au moment de
    -- l'infection ("un villageois a secrètement rejoint les Loups-Garous") —
    -- exposer CE SEUL booléen ne révèle ni qui a été infecté ni qui est
    -- l'Alpha, juste le fait qu'une conversion a eu lieu quelque part dans la
    -- partie. Un Loup Alpha ne peut infecter qu'une seule fois par partie
    -- (alpha_infect_used, déjà en place), donc ce booléen suffit : jamais
    -- plus d'une conversion possible. Sert au client (RosterSummary.tsx) à
    -- corriger le total de loups affiché, qui sinon resterait figé sur la
    -- composition INITIALE (role_counts) même après une infection réussie.
