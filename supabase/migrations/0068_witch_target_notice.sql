-- ============================================================================
-- Aujourd'hui, quand la Sorcière utilise une de ses potions, la victime
-- concernée n'en est jamais informée personnellement :
--   - potion de vie (heal) : le journal public dit juste "la Sorcière a
--     sauvé la victime des loups", SANS jamais dire qui — même la personne
--     sauvée ne sait donc pas qu'elle a été prise pour cible cette nuit-là ;
--   - potion de mort (poison) : kill_player logue déjà "Untel a été
--     empoisonné par la Sorcière", mais ce n'est qu'une ligne noyée parmi
--     les autres dans le récap public, jamais mise en avant pour la victime
--     elle-même (contrairement à lover_id/mentee_ids, qui ont chacun leur
--     encart dédié dans NightRecapModal, voir migration 0061).
--
-- Ce correctif :
-- 1) resolve_night_deaths (reprise de 0043_night_recap_headlines.sql) tague
--    désormais le log de soin avec kind='witch_heal' et
--    meta={'target_user_id': <victime sauvée>} — la ligne de journal
--    publique affichée à tout le monde ne change pas, seul un identifiant
--    cause invisible (kind/meta ne sont jamais renvoyés dans night_recap/log)
--    est ajouté pour permettre de reconnaître la cible côté serveur.
-- 2) get_my_game_view (reprise de 0061_lover_and_mentor_notifications.sql)
--    expose deux nouveaux booléens PERSONNELS (calculés à partir de
--    auth.uid(), jamais diffusés à qui que ce soit d'autre) :
--    witch_saved_me / witch_poisoned_me, vrais uniquement pendant le récap
--    de la nuit concernée (statut 'day_reveal', night_number correspondant).
-- ============================================================================
...
begin
  ...
  if v_heal is true and v_final_victim is not null then
    -- kind='witch_heal' + meta.target_user_id : uniquement exploité par
    -- get_my_game_view (witch_saved_me ci-dessous) pour prévenir la
    -- personne concernée — le message public ci-dessous reste inchangé et
    -- anonyme pour tout le reste du village.
...
-- ----------------------------------------------------------------------------
-- get_my_game_view : reprise intégrale de 0061_lover_and_mentor_notifications.sql,
-- ajoute witch_saved_me / witch_poisoned_me juste après mentee_ids.
-- ----------------------------------------------------------------------------
...
begin
  ...
  select jsonb_build_object(
    ...
    -- Personnel, jamais diffusé à qui que ce soit d'autre : est-ce que MOI
    -- (v_user) ai été la cible de la potion de vie / de mort de la Sorcière
    -- CETTE nuit — seul moment où NightRecapModal doit l'afficher (statut
    -- 'day_reveal', nuit courante). witch_saved_me lit le tag posé par
    -- resolve_night_deaths ci-dessus ; witch_poisoned_me se déduit
    -- directement de death_cause, déjà écrit par kill_player.
