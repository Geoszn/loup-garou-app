-- ============================================================================
-- Notification pour le joueur PROTÉGÉ par le Daron (retour utilisateur) :
-- jusqu'ici, être choisi comme cible de protection cette nuit-là était
-- totalement invisible pour la personne concernée — seul le Daron lui-même
-- (daron_protected_id/daron_protection_worked, voir migration 0158) savait
-- qui il protégeait. Ajoute une révélation ponctuelle et strictement
-- personnelle dans le récap de nuit (NightRecapModal), pour QUE la cible
-- sache qu'elle a été protégée cette nuit-là.
--
-- Volontairement inconditionnel (dès que choisi(e) comme cible, que la
-- protection ait servi contre une attaque ou non) plutôt que réservé aux cas
-- où elle a réellement bloqué quelque chose — décision utilisateur explicite
-- ("quand le daron protège quelqu'un... cette personne reçoit la notif
-- qu'elle a été protégée"), pas besoin de modifier resolve_night_deaths
-- pour ça : une simple lecture de night_actions (step='daron', target_id =
-- moi) suffit, exactement comme daron_protected_id le fait déjà côté Daron.
--
-- Ne nomme jamais le Daron ni ne révèle son identité au joueur protégé —
-- même logique de discrétion que le message public existant (voir
-- commentaire de tête, migration 0158) et que witch_saved_me/mentorReveal
-- pour les autres rôles à identité secrète.
--
-- Réservé aux joueurs qui NE SONT PAS le Daron : quand il se protège
-- lui-même (auto-protection autorisée), il a déjà son propre encart dédié
-- (daronProtectedTarget/daron_protection_worked, NightRecapModal.tsx) —
-- pas besoin d'un second message redondant pour la même information.
--
-- get_my_game_view inchangée sinon (identique à 0163) : simple ajout inline
-- d'un nouveau champ, même patron que daron_protected_id/daron_protection_worked
-- juste au-dessus.
-- ============================================================================
...
begin
  ...
  select (
    jsonb_build_object(
      ...
      'my_protected_by_daron_this_round', case when v_my_role <> 'daron' and v_game.status = 'day_reveal' then exists (
