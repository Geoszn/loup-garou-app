-- ============================================================================
-- Demande utilisateur : "Revois la carte du voleur. Il n'est pas sensé
-- choisir entre loup et villageois. Il choisit juste un joueur au hasard sans
-- connaître sa carte et se retrouve à échanger sa carte avec ce joueur. Mais
-- bien évidemment immédiatement lors de la première nuit le joueur sera
-- informé que le voleur lui a volé sa carte et maintenant quel est son
-- nouveau rôle."
--
-- Ancien mécanisme : le Voleur voyait 2 cartes "en trop" (Loup-Garou +
-- Villageois, thief_extra_roles) et choisissait librement d'échanger sa
-- propre carte contre l'une des deux, ou de garder la sienne.
--
-- Nouveau mécanisme : le Voleur ne voit rien et ne choisit personne. Il
-- confirme juste son action ("Voler une carte"), et le serveur tire au sort
-- un autre joueur vivant. Les deux cartes (Voleur <-> victime) sont
-- échangées à l'aveugle : le Voleur hérite du rôle (et du camp) de la
-- victime, la victime devient "voleur" (rôle inerte pour le reste de la
-- partie, comme avant quand le Voleur choisissait de garder sa carte).
-- Comme le Voleur agit en tout premier à la nuit 1 (voir next_night_step,
-- séquence night_number <= 1), l'échange est terminé avant que quiconque
-- d'autre n'agisse cette nuit-là -- aucun risque d'incohérence avec les
-- autres rôles (ex: si la victime devient Sorcière/Voyante par l'échange,
-- elle jouera bien son tour normalement plus tard cette même nuit).
--
-- La victime est prévenue : un événement privé (kind='thief_swap', meta avec
-- victim_id + new_role) est inséré dans game_log. Comme le champ générique
-- `log` de get_my_game_view renvoie le texte de TOUS les messages à TOUS les
-- joueurs sans filtrage par kind/meta, le message PUBLIC associé à cette
-- ligne reste volontairement générique/anonyme (identique au message déjà
-- existant) -- seule get_my_game_view calcule, pour l'utilisateur concerné
-- uniquement, deux champs privés thief_stole_my_card / thief_stole_my_new_role
-- à partir du meta.
-- ============================================================================
set search_path = public;

-- --- start_game : ne prépare plus les 2 cartes "en trop" du Voleur ---------
...
-- --- submit_voleur : nouvelle signature, plus de choix de carte ------------
drop function if exists public.submit_voleur(uuid, text);
...
begin
  ...
  if v_target is not null then
    ...
    -- Message public inchangé et anonyme (voir en-tête de fichier) ; le
    -- meta ci-dessous n'est lu que par get_my_game_view pour LA victime.
    ...
    values (
      ...
      v_game.night_number,
      'thief_swap',
      jsonb_build_object('victim_id', v_target, 'new_role', v_my_role)
    );
  else
...
-- --- get_my_game_view : notifie la victime en privé -------------------------
