-- ============================================================================
-- Le récap de nuit (pop-up day_reveal, voir migration 0041/NightRecapModal)
-- affichait jusqu'ici les 6 dernières lignes du journal GLOBAL de la
-- partie (view.log.slice(0,6)) — trop verbeux, et pas toujours pertinent
-- (peut remonter jusqu'à un message de la phase précédente si peu de choses
-- se sont passées cette nuit-là).
--
-- Plutôt qu'un filtrage fragile par mots-clés côté client, on tague
-- précisément à la source (colonne game_log.night_number) les seules
-- entrées produites PAR la résolution de la nuit elle-même — kill_player et
-- resolve_night_deaths reçoivent déjà le numéro de nuit en paramètre, il ne
-- manquait que la colonne pour le conserver. Tout le reste du journal
-- (messages de phase, arrivées/départs, etc.) garde night_number = null et
-- n'apparaît donc jamais dans ce récap précis.
-- ============================================================================
set search_path = public;

alter table public.game_log add column if not exists night_number int;

-- ----------------------------------------------------------------------------
-- kill_player : reprise de 0018_captain.sql, seule différence — chaque
-- insert dans game_log précise désormais night_number = p_night.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- resolve_night_deaths : reprise de 0032_remove_petite_fille_spy.sql, même
-- différence — night_number = v_night sur ses deux inserts.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if v_heal is true and v_final_victim is not null then
    insert into public.game_log (game_id, message, night_number)
    values (p_game_id, '🧪 La Sorcière a utilisé sa potion de guérison pour sauver la victime des loups.', v_night);
...
-- ----------------------------------------------------------------------------
-- get_my_game_view : reprise de 0041, ajoute `night_recap` — uniquement les
-- entrées taguées night_number = nuit courante, uniquement pendant
-- 'day_reveal' (inutile ailleurs). Remplace l'usage de `log.slice(0,6)` côté
-- client (voir NightRecapModal.tsx).
-- ----------------------------------------------------------------------------
