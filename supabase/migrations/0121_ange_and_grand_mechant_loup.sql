-- ----------------------------------------------------------------------------
-- Ajoute deux rôles célèbres de l'extension "Nouvelle Lune"/"Personnages" :
--
-- 👼 L'Ange (camp village, passif) : ne fait rien la nuit. Variante maison
-- demandée explicitement par l'utilisateur, plus généreuse que la règle
-- officielle (qui exige d'être éliminé PAR LE VILLAGE au premier vote) —
-- ici, mourir de N'IMPORTE QUELLE cause pendant le tout premier cycle
-- (nuit 1 OU jour 1, jusqu'au vote inclus) suffit à gagner seul, aussitôt,
-- quel que soit l'état du reste de la partie. died_at_night = 1 couvre les
-- deux cas d'un coup (kill_player pose died_at_night = p_night aussi bien
-- pour une mort de nuit que pour le vote du jour qui la suit, tant que
-- night_number n'a pas encore avancé — voir begin_night, appelé seulement
-- après le récap du vote). S'il survit ce premier cycle, il perd
-- définitivement son pouvoir et redevient un villageois ordinaire pour le
-- reste de la partie (aucune transformation nécessaire : il n'a jamais eu
-- d'action, donc rien à désactiver — juste ne plus jamais re-déclencher la
-- vérification de victoire au-delà de ce point, ce qui est déjà le cas
-- puisque check_and_apply_ange_win n'est appelée qu'à la résolution de la
-- nuit 1 et du vote du jour 1).
--
-- 👹 Le Grand Méchant Loup (camp loups) : vote avec la meute comme un loup
-- simple (WolfPanel générique, aucun changement client nécessaire), PUIS
-- peut dévorer une seconde victime — différente de celle de la meute,
-- jamais un coéquipier loup — seul, sans consulter personne. Ce pouvoir
-- n'existe QUE tant qu'aucun loup n'est encore mort dans la partie (loup
-- simple, Alpha, Sans-Visage ou lui-même) ; dès qu'un loup meurt, le
-- pouvoir disparaît pour de bon (next_night_step ne propose plus jamais
-- l'étape ensuite). Choix explicite de l'utilisateur : sa seconde victime
-- est invisible pour la Sorcière et donc impossible à sauver (contrairement
-- à la victime classique de la meute) — c'est une frappe garantie tant que
-- le pouvoir est actif.
-- ----------------------------------------------------------------------------

-- 1. Constraintes : nouvelle étape de nuit + nouveau camp vainqueur.
...
alter table public.games add constraint games_night_step_check
  check (night_step = any (array[
    'voleur','cupidon','enfant_sauvage','voyante','griot',
...
alter table public.games add constraint games_winner_team_check
  check (winner_team = any (array['village','loups','amoureux','anancy','ange']));

-- ----------------------------------------------------------------------------
-- 2. compute_default_role_counts / start_game : nouvelles cases, désactivées
-- par défaut, comme tous les rôles spéciaux introduits depuis le Griot.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. next_night_step : insère 'grand_mechant_loup' juste après 'loup_garou',
-- avec sa propre condition d'apparition (rôle vivant ET aucun loup mort).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. submit_wolf_vote / submit_alpha_infect_agreement /
-- submit_loup_alpha_confirm_infect : le Grand Méchant Loup participe au vote
-- collectif de la meute (et à la décision d'infection de l'Alpha, le cas
-- échéant) exactement comme un loup simple.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if public.my_role_in_game(p_game_id) not in ('loup_garou', 'loup_alpha', 'sans_visage', 'grand_mechant_loup') then
  ...
  if p_target is not null then
    ...
    if v_target_role in ('loup_garou', 'loup_alpha', 'sans_visage', 'grand_mechant_loup') then
...
begin
  ...
  if v_role not in ('loup_garou', 'sans_visage', 'grand_mechant_loup') then
...
-- ----------------------------------------------------------------------------
-- 5. resolve_night_deaths : la seconde victime du Grand Méchant Loup est
-- résolue APRÈS le vote de meute et la potion de la Sorcière — jamais
-- affectée par la potion de guérison (choix explicite de l'utilisateur :
-- invisible pour la Sorcière), toujours une mise à mort directe (jamais une
-- infection).
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- à la potion de guérison de la Sorcière (déjà résolue ci-dessus, sans le
  -- moindre effet sur cette cible).
...
-- ----------------------------------------------------------------------------
-- 6. submit_grand_mechant_loup : la seconde victime, choisie seul.
-- ----------------------------------------------------------------------------

create or replace function public.submit_grand_mechant_loup(p_game_id uuid, p_target uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  select * into v_game from public.games where id = p_game_id for update;
  if not found or v_game.status <> 'night' or v_game.night_step <> 'grand_mechant_loup' then
    raise exception 'Ce n''est pas le moment pour cette décision.';
  end if;
  if public.my_role_in_game(p_game_id) <> 'grand_mechant_loup' then
    raise exception 'Vous n''êtes pas le Grand Méchant Loup.';
  end if;
  if not exists (select 1 from public.game_players where game_id = p_game_id and user_id = v_user and is_alive) then
    raise exception 'Vous êtes mort.';
  end if;
  ...
  if p_target is not null then
    ...
    v_pack_target := public.get_wolf_target(p_game_id, v_game.night_number);
    if v_pack_target is not null and p_target = v_pack_target then
      raise exception 'Choisissez une victime différente de celle de la meute.';
    end if;
  end if;

  insert into public.night_actions (game_id, night_number, step, actor_id, target_id)
  values (p_game_id, v_game.night_number, 'grand_mechant_loup', v_user, p_target)
  on conflict (game_id, night_number, step, actor_id) do update set target_id = excluded.target_id;
...
grant execute on function public.submit_grand_mechant_loup(uuid, uuid) to authenticated;

-- ----------------------------------------------------------------------------
-- 7. check_and_apply_ange_win : appelée à la résolution de la nuit 1 ET du
-- vote du jour 1 (voir advance_phase, section 8) — died_at_night = 1 couvre
-- les deux en un seul test.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 8. advance_phase : appelle check_and_apply_ange_win à la résolution de la
-- nuit ET du vote du jour, avant les vérifications de victoire habituelles
-- (même priorité que check_and_apply_anancy_win — si l'Ange meurt au tour 1,
-- sa victoire prime sur toute autre conclusion simultanée).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 9. infect_player : un villageois infecté ne peut jamais déjà être un loup
-- (garde défensive, inchangée dans son principe — juste étendue à
-- grand_mechant_loup pour rester cohérente avec les autres listes).
-- ----------------------------------------------------------------------------
...
begin
  ...
  if v_old_role is null or v_old_role in ('loup_garou', 'loup_alpha', 'sans_visage', 'grand_mechant_loup') then
...
-- ----------------------------------------------------------------------------
-- 10. can_access_channel / can_read_channel : accès au chat des loups.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 11. check_and_apply_win : décompte des loups vivants (aucun changement
-- pour 'ange', qui n'a plus le moindre statut spécial une fois passé le
-- premier cycle — il retombe naturellement dans le camp village).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 12. apply_rank_updates_for_game : nouvelle branche 'ange' (tout le monde
-- d'autre perd, même principe qu'Anancy) + grand_mechant_loup dans les
-- listes "camp des loups".
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 13. compute_impact_bonus : Grand Méchant Loup rejoint le bloc "camp des
-- loups" (villageois dévorés, nuits survécues) + bonus dédié s'il a réussi
-- au moins une seconde victime pendant la partie.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 14. get_leaderboard : décompte des victoires "loups"/"village" + nouveau
-- camp 'ange'.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 15. role_display_name : libellés utilisés dans les messages de log.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 16. sync_daily_quests_for_game : quêtes "gagné en tant que loup/village".
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 17. compute_griot_phrase : le Griot reconnaît le vote de meute du Grand
-- Méchant Loup comme n'importe quel loup ; sa seconde victime personnelle
-- (étape distincte) réutilise la phrase générique "used_power" déjà
-- utilisée pour la Sorcière — jamais de détail qui trahirait son rôle
-- exact, conformément à l'esprit du Griot.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if v_role = 'grand_mechant_loup' and exists (
    select 1 from public.night_actions
  ...
  if v_role in ('loup_garou', 'loup_alpha', 'sans_visage', 'grand_mechant_loup') and exists (
    select 1 from public.night_actions
...
-- ----------------------------------------------------------------------------
-- 18. get_my_game_view : Grand Méchant Loup rejoint toutes les classifications
-- "camp des loups" déjà utilisées pour Sans-Visage (coéquipiers, chat,
-- accord/confirmation d'infection de l'Alpha, votes en direct, récap de nuit,
-- exception pending_action_required pour le vote de meute) + révélé comme
-- 'loup_garou' par la Voyante (contrairement au Sans-Visage, jamais caché
-- d'elle). Aucun ajout pour l'Ange : rôle entièrement passif, sans la
-- moindre donnée à exposer.
-- ----------------------------------------------------------------------------
