...
export function ModerationPanel({ view, gameId, selfId }: { view: MyGameView; gameId: string; selfId: string }) {
  ...
  const [transferTarget, setTransferTarget] = useState<PublicPlayer | null>(null)
  ...
  // Transfert volontaire de l'hôte (migration 0160) : réservé au salon
  // d'attente (voir isLobbyOrEnded plus bas — non, précisément
  // view.game.status === 'lobby', 'ended' exclu ici contrairement à
  // isLobbyOrEnded : une fois la partie terminée, l'écran de fin a son
  // propre bouton "rejouer" qui relance un tout nouveau salon, transférer
  // l'hôte de l'ancien n'a plus de sens). Le serveur revalide déjà tout
  // (statut lobby, cible non bannie, cible pas un bot) — cette page ne fait
  // que refléter la même logique côté affichage.
  async function confirmTransfer() {
    if (!transferTarget) return
    setTransferring(true)
    setError(null)
    const { error: rpcError } = await supabase.rpc('transfer_host', {
      p_game_id: gameId,
      p_new_host_id: transferTarget.user_id,
    })
  ...
  // Jamais un bot comme cible : pas de session réelle pour agir comme
  // hôte, ça verrouillerait le salon (voir migration 0160). Même heuristique
  // que le reste de l'appli (Lobby.tsx) — aucune colonne dédiée "is_bot".
  const transferableOthers = others.filter((p) => !p.display_name.startsWith('🤖 '))
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Transfert volontaire de l'hôte (migration 0160) : uniquement dans
          le salon d'attente, jamais en partie ni une fois terminée — voir
          confirmTransfer ci-dessus pour le détail. Bots exclus de la liste
          (transferableOthers). */}
      {view.game.status === 'lobby' && (
        <div>
          <h3 className="mb-2 font-display text-sm text-moon-300">{t('moderation.transferHostTitle')}</h3>
          {transferableOthers.length === 0 ? (
            <p className="text-xs text-moon-200/40">{t('moderation.noOtherPlayers')}</p>
          ) : (
            <ul className="flex flex-col gap-1.5">
              {transferableOthers.map((p) => (
                <li
                  key={p.id}
                  className="flex items-center justify-between rounded-xl border border-night-600/60 bg-night-900/40 px-3 py-2 text-sm"
                >
                  ...
                  <Button variant="ghost" className="shrink-0 px-2.5 py-1 text-xs" onClick={() => setTransferTarget(p)}>
                    {t('moderation.transferButton')}
                  </Button>
                </li>
              ))}
            </ul>
      ...
      <ConfirmDialog
        ...
        message={t('moderation.transferConfirmMessage', { name: transferTarget?.display_name ?? '' })}
